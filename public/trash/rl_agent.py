...
class RLGameAgent:
    def __init__(self, server_url="ws://localhost:8787"):  # Connect to your Node.js server
        ...
        try:
            ...
            model_paths = ["models/ppo_model.zip", "ppo_model.zip"]
            self.model = None
            ...
            if self.model is None:
                print("⚠️  No pre-trained model found. Using heuristic AI instead.")
                print("Run 'python download_model.py' to download a pre-trained model.")
                
        except Exception as e:
            print(f"❌ Error loading model: {e}. Using heuristic AI instead.")
            self.model = None
    ...
    def create_state_vector(self, player, enemies, bullets):
        ...
        - Player: x, y, health, ult_charge, ammo, reloading_status (6D)
        - Nearest enemy: rel_x, rel_y, health (3D)
        - Nearest bullet: rel_x, rel_y (2D)
        """
        ...
        state = [
            ...
            player.get('ammo', 30) / 30,  # Normalized ammo (assuming max 30)
            1.0 if player.get('reloading', False) else 0.0,  # Reloading status
        ]
    ...
    def get_action(self, state):
        ...
        - Movement: vx, vy (continuous -1 to 1)
        - Actions: shoot, dash, ult, shield, reload (boolean)
        """
        if self.model:
    ...
    def simple_heuristic(self, state):
        ...
        player_x, player_y, health, ult, ammo, reloading = state[0:6]
        enemy_rel_x, enemy_rel_y, enemy_health = state[6:9]
        bullet_rel_x, bullet_rel_y = state[9:11]
        ...
        # Reload if low on ammo and not currently reloading
        if ammo < 0.3 and not reloading:
            actions['reload'] = True
        
        # Shoot if enemy is close and have ammo and not reloading
        if abs(enemy_rel_x) < 0.3 and abs(enemy_rel_y) < 0.3 and ammo > 0 and not reloading:
            actions['shoot'] = True
        ...
        # Use shield when low health and enemy/bullets nearby
        if health < 0.5 and (abs(enemy_rel_x) < 0.2 or abs(bullet_rel_x) < 0.15):
            actions['shield'] = True
    ...
    async def send_action(self, action):
        ...
        action_message = {
            ...
            "input": {  # Note: wrapped in 'input' object
                ...
                "reload": action.get('reload', False),
